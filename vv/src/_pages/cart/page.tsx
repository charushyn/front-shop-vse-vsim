...
import {
  ...
  incrementItem,
  removeItem,
} from "@/shared/utils/redux/productCart/productCart";
...
const formSchema = z.object({
  ...
  phone: z.string().refine((value) => isValidPhoneNumber(value, "UA"), {
...
import {
  getCity,
  getDepartament,
} from "@/shared/utils/api/novapost/client/novapost-client";
import {
  saveCounterParty,
  saveContactPerson,
  saveInternetDocument,
} from "@/shared/utils/api/novapost/server/novapost-server";
...
import {
  saveOrder,
  updateOrderShippingAdress,
} from "@/shared/utils/api/crm/order";
...
import makeOrder from "@/shared/utils/api/scenario/makeOrder";
...
export default function CartPage() {
  ...
  async function onSubmit(values: z.infer<typeof formSchema>) {
    ...
    try {
      ...
      // const orderStatus = await makeOrder({
      //   ...values,
      //   shipping_day: "16",
      //   shipping_mounth: "11",
      //   shipping_year: "2024",
      //   finalCount: finalCount,
      //   products: cart.map((cartItem: any) => {
      //     return {
      //       sku: cartItem.item.sku,
      //       quantity: cartItem.quantity,
      //     };
  ...
  return (
    <div className="flex flex-col px-4 min-h-svh">
      ...
      >
        ...
        <TypographyP>Назад</TypographyP>
      </div>
      <TypographyH3 className="mt-2 mb-6 font-medium">Кошик:</TypographyH3>
      {cart.length > 0 ? (
        cart.map(
          (product: { item: Product; quantity: number }, index: number) => {
            return (
              <CartItem
                product={product}
                funcMinus={() => {
                ...
                funcRemove={() => dispath(removeItem({ index: index }))}
              ></CartItem>
            );
          }
        )
      ) : (
        <TypographyLarge className="text-center mt-10">
          Пусто...
        </TypographyLarge>
      )}
      {cart.length > 0 && (
        <div className="flex flex-row gap-2 mb-10 justify-end">
          <TypographyLarge className="font-[400]">Загалом:</TypographyLarge>
          <TypographyLarge className=" font-bold">{`${finalCount}.00 ₴`}</TypographyLarge>
        </div>
      )}
      {finalCount !== 0 && finalCount < 1999 && (
        <div className=" bg-main text-white flex flex-row justify-center p-4 rounded-2xl gap-4 items-center mb-10 t-s:w-[350px] self-end">
          ...
          <TypographyLarge className="w-[90%]">
            Наповніть кошик від 1999 грн та насолоджуйтесь безкоштовною
            доставкою!
          </TypographyLarge>
        </div>
      )}
      {finalCount > 1999 && (
        <div className=" bg-light_green text-white flex flex-row justify-center p-4 rounded-2xl gap-4 items-center mb-10 t-s:w-[350px] self-end">
          ...
          <TypographyLarge className="w-[80%]">
        ...
        <div className="flex justify-center mb-10" id="orderform">
          <Form {...form}>
            ...
            >
                    ...
                    <FormControl>
                      <Input
                        ...
                        onChangeCapture={async (e) => {
                          const res = (
                            await getCity({ cityName: e.currentTarget.value })
                          )?.data.data;
                      ...
                      {citySelect[0]?.Addresses.map((item: any) => {
                        if (item.AddressDeliveryAllowed) {
                          return (
                            ...
                            >
                              <TypographyP>{item.Present}</TypographyP>
                            </div>
                          );
                        }
                      })}
                    ...
                    <FormItem>
                      ...
                      <FormControl>
                        <Input
                          placeholder=""
                          onChangeCapture={async (e) => {
                            const res = await getDepartament({
                              cityRef: form.getValues("cityObject.Ref"),
              ...
              {form.getValues("departamentObject.Ref") &&
                form.getValues("cityObject.Ref") && (
                  <FormField
                    control={form.control}
                    name="PaymentMethod"
